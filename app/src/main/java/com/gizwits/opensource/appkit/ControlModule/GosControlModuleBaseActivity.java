...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 * ===========================================================
	 */
	// 数据点"ir_detect"对应的标识名
	protected static final String KEY_IR_DETECT = "ir_detect";
	// 数据点"swith"对应的标识名
	protected static final String KEY_SWITH = "swith";
	// 数据点"vol_liquid"对应的标识名
	protected static final String KEY_VOL_LIQUID = "vol_liquid";
	 ...
	 * ===========================================================
	 */
	// 数据点"ir_detect"对应的存储数据
	protected static boolean ir_detect;
	// 数据点"swith"对应的存储数据
	protected static boolean swith;
	// 数据点"vol_liquid"对应的存储数据
	protected static int vol_liquid;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_IR_DETECT)) {
					ir_detect = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_SWITH)) {
					swith = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_VOL_LIQUID)) {
			
					vol_liquid = (Integer) map.get(dataKey);
				}
			}
		}
